...
exports.page = function(request, response) {
  db.query(`SELECT * FROM topic`, function(error, topics) {  //callback:sql문이 실행된 후에 서버가 응답한 결과를 처리해줌
    ...
    db.query(`SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id WHERE topic.id=?`, [request.params.pageId], function(error2, topic) { //보안을 위해 sql문에 ?로 두번째 인자가 치환되도록 함(?은 문자가 돼서 DROP문을 입력해도 문자로 처리해서 공격을 막을 수 있음)
...
exports.update_process = function(request, response) {
  ...
  request.on('end', function() {
    ...
    db.query(`UPDATE topic SET title=?, description=?, author_id=? WHERE id=?`, [post.title, post.description, post.author, post.id], function(error, result){
      //response.writeHead(302, {Location: `/page/${post.id}`});  //리다이렉션(Location으로 이동)
      //response.end();
